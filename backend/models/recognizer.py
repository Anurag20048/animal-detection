...
class BiometricRecognizer:
    ...
    def compare_to_animal(self, animal_id: str, embeddings: Mapping[str, np.ndarray]) -> RecognitionMatch:
        ...
        if not reference:
            return RecognitionMatch(animal_id=animal_id, similarity=0.0, region_scores={})
    ...
    def find_best_match(self, animal_type: str, embeddings: Mapping[str, np.ndarray]) -> RecognitionMatch:
        ...
        for animal_id, reference in self.embeddings.items():
            if self.animal_types.get(animal_id) != animal_type:
                continue
            score, region_scores = weighted_similarity(embeddings, reference, weights=self.region_weights)
    ...
    def serialize_record(self, animal_id: str) -> Dict[str, object]:
        return {
            "animal_id": animal_id,
            "animal_type": self.animal_types.get(animal_id, "Other Animals"),
            "embeddings": {
                region: vector.astype(float).tolist()
                for region, vector in self.embeddings.get(animal_id, {}).items()
            },
        }
    ...
    def load_record(self, record: Mapping[str, object]) -> None:
        ...
        if not animal_id or not isinstance(raw_embeddings, dict):
            return
        embeddings: Dict[str, np.ndarray] = {}
        for region, vector in raw_embeddings.items():
            try:
                embeddings[str(region)] = normalize_embedding(np.asarray(vector, dtype=np.float32))
            except Exception:
                continue
        if embeddings:
            self.register(animal_id, animal_type, embeddings)
